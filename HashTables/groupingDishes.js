...
the first element of dishes[i] is the name of the dish and the following elements are the ingredients
 of that dish. Both the dish name and the ingredient names consist of English letters and spaces.
  It is guaranteed that all dish names are different. It is also guaranteed that ingredient names 
  for one dish are also pairwise different.
  ...
  dishes.forEach(dish => {
    ...
    dish.forEach(ing => {
      if (ings[ing]) {
        ...
        ings[ing] = new Set([name]);
      }
    });
  });

  return Object.keys(ings).sort().reduce((acc, ing) => {
    if (ings[ing].size > 1) {
      acc.push([ing, ...Array.from(ings[ing]).sort()]);
    }
    return acc;
  }, []);
}